...
namespace DataView2.Core.Models.LCMS_Data_Tables
{
    ...
    public class LCMS_PCI : IEntity
    {
        ...
        [DataMember(Order = 6)]
        public string DeductedValues { get; set; }
        [DataMember(Order = 7)]
    ...
    {
        [OperationContract]
        Task<IdReply> Create(LCMS_PCI request, CallContext context = default);

        [OperationContract]
        Task<List<LCMS_PCI>> GetAll(Empty empty, CallContext context = default);

        [OperationContract]
        Task<LCMS_PCI> GetById(IdRequest request, CallContext context = default);

        [OperationContract]
        Task<LCMS_PCI> EditValue(LCMS_PCI request, CallContext context = default);

        [OperationContract]
        Task<IdReply> DeleteAll(Empty empty, CallContext context = default);

        [OperationContract]
        Task<List<LCMS_PCI>> GetBySurvey(SurveyRequest request);

        [OperationContract]
        Task<IEnumerable<LCMS_PCI>> QueryAsync(string predicate);
