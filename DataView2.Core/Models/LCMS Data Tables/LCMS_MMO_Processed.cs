...
namespace DataView2.Core.Models.LCMS_Data_Tables
{
    //man made objects
    [DataContract]
    public class LCMS_MMO_Processed: IEntity
    {
        [DataMember(Order = 1)]
        [Key]
    ...
    {
        [OperationContract]
        Task<IdReply> Create(LCMS_MMO_Processed request, CallContext context = default);

        [OperationContract]
        Task<List<LCMS_MMO_Processed>> GetAll(Empty empty, CallContext context = default);

        [OperationContract]
        Task<LCMS_MMO_Processed> GetById(IdRequest request, CallContext context = default);
        [OperationContract]
        Task<LCMS_MMO_Processed> EditValue(LCMS_MMO_Processed request, CallContext context = default);
        [OperationContract]
        Task<IdReply> DeleteAll(Empty empty, CallContext context = default);
        [OperationContract]
        Task<List<LCMS_MMO_Processed>> GetBySurvey(SurveyRequest request);
        [OperationContract]
        Task<IEnumerable<LCMS_MMO_Processed>> QueryAsync(string predicate);
        [OperationContract]
        ...
        [OperationContract]
        Task<IdReply> DeleteObject(LCMS_MMO_Processed request, CallContext context = default);

        [OperationContract]
        Task<CountReply> GetRecordCount(Empty empty, CallContext context = default);
        [OperationContract]
        Task<LCMS_MMO_Processed> UpdateGenericData(string fieldsToUpdateSerialized);
    }
}
