...
{
    public class PatchService: BaseService<LCMS_Patch_Processed, IRepository<LCMS_Patch_Processed>>, IPatchService 
    {
        private readonly AppDbContextProjectData _context;
        IDbContextFactory<AppDbContextProjectData> _dbContextFactory;

        public PatchService(IRepository<LCMS_Patch_Processed> repository, IDbContextFactory<AppDbContextProjectData> dbContextFactor) : base(repository)
        {
        ...
        public async Task SaveToDatabase(List<LCMS_Patch_Processed> entities, CancellationToken cancellation)
        {
		...
		public async Task<IEnumerable<LCMS_Patch_Processed>> QueryAsync(string predicate)
        {
            try
            {
                var sqlQuery = predicate;
                var lstTables = await _context.LCMS_Patch_Processed.FromSqlRaw(sqlQuery).ToListAsync();
            ...
            catch (Exception ex)
            {
                ...
                return new List<LCMS_Patch_Processed>();
            }
        }
        public async Task<IEnumerable<int>> QueryIdsAsync(string predicate)
        {
            ...
            {
                var sqlQuery = predicate;
                var ids = await _context.LCMS_Patch_Processed.FromSqlRaw(sqlQuery)
                                                          .Select(item => item.Id)
                                                          .ToListAsync();
        ...
        public async Task<CountReply> GetCountAsync(string sqlQuery)
        {
            try
            {
                var count = await _context.LCMS_Patch_Processed.FromSqlRaw(sqlQuery).CountAsync();
        ...
        public async Task<LCMS_Patch_Processed> UpdateGenericData(string fieldsToUpdateSerialized)
        {
            var entity = new LCMS_Patch_Processed();
