...
{
    public class SpallingRawService: BaseService<LCMS_Spalling_Raw, IRepository<LCMS_Spalling_Raw>>, ISpallingService 
    {
        private readonly AppDbContextProjectData _context;
        IDbContextFactory<AppDbContextProjectData> _dbContextFactory;

        public SpallingRawService(IRepository<LCMS_Spalling_Raw> repository, IDbContextFactory<AppDbContextProjectData> dbContextFactor) : base(repository)
        {
		...
		public async Task<IEnumerable<LCMS_Spalling_Raw>> QueryAsync(string predicate)
        {
            try
            {
                var sqlQuery = predicate;
                var lstTables = await _context.LCMS_Spalling_Raw.FromSqlRaw(sqlQuery).ToListAsync();
            ...
            catch (Exception ex)
            {
                ...
                return new List<LCMS_Spalling_Raw>();
            }
        }
        public async Task<IEnumerable<int>> QueryIdsAsync(string predicate)
        {
            ...
            {
                var sqlQuery = predicate;
                var ids = await _context.LCMS_Spalling_Raw.FromSqlRaw(sqlQuery)
                                                          .Select(item => item.Id)
                                                          .ToListAsync();
        ...
        public async Task<CountReply> GetCountAsync(string sqlQuery)
        {
            try
            {
                var count = await _context.LCMS_Spalling_Raw.FromSqlRaw(sqlQuery).CountAsync();
        ...
        public async Task<LCMS_Spalling_Raw> UpdateGenericData(string fieldsToUpdateSerialized)
        {
            var entity = new LCMS_Spalling_Raw();
