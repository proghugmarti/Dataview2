...
{
    public class SealedCrackService : BaseService<LCMS_Sealed_Cracks, IRepository<LCMS_Sealed_Cracks>>, ISealedCrackService
    {
        private readonly AppDbContextProjectData _context;
        IDbContextFactory<AppDbContextProjectData> _dbContextFactory;

        public SealedCrackService(IRepository<LCMS_Sealed_Cracks> repository, IDbContextFactory<AppDbContextProjectData> dbContextFactor) : base(repository)
        {
            _dbContextFactory = dbContextFactor;
            _context = _dbContextFactory.CreateDbContext();
        }

        public async Task<IEnumerable<LCMS_Sealed_Cracks>> QueryAsync(string predicate)
        {
            try
            {
                var sqlQuery = predicate;
                var lstTables = await _context.LCMS_Sealed_Cracks.FromSqlRaw(sqlQuery).ToListAsync();
            ...
            catch (Exception ex)
            {
                ...
                return new List<LCMS_Sealed_Cracks>();
            }
        }
        ...
        public async Task<CountReply> GetCountAsync(string sqlQuery)
        {
            try
            {
                var count = await _context.LCMS_Sealed_Cracks.FromSqlRaw(sqlQuery).CountAsync();
        ...
        public async Task<LCMS_Sealed_Cracks> UpdateGenericData(string fieldsToUpdateSerialized)
        {
            var entity = new LCMS_Sealed_Cracks();
