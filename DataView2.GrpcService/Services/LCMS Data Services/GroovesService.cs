...
{
    public class GroovesService: BaseService<LCMS_Grooves, IRepository<LCMS_Grooves>>, IGroovesService
    {
        private readonly AppDbContextProjectData _context;
        IDbContextFactory<AppDbContextProjectData> _dbContextFactory;

        public GroovesService(IRepository<LCMS_Grooves> repository, IDbContextFactory<AppDbContextProjectData> dbContextFactor) : base(repository)
        {
            _dbContextFactory = dbContextFactor;
            _context = _dbContextFactory.CreateDbContext();
        }

        public async Task<IEnumerable<LCMS_Grooves>> QueryAsync(string predicate)
        {
            try
            {
                var sqlQuery = predicate;
                var lstTables = await _context.LCMS_Grooves.FromSqlRaw(sqlQuery).ToListAsync();
        ...
        public async Task<CountReply> GetCountAsync(string sqlQuery)
        {
            try
            {
                var count = await _context.LCMS_Grooves.FromSqlRaw(sqlQuery).CountAsync();
        ...
        public async Task<LCMS_Grooves> UpdateGenericData(string fieldsToUpdateSerialized)
        {
            var entity = new LCMS_Grooves();
