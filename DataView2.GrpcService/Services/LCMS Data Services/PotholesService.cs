...
{
    public class PotholesService: BaseService<LCMS_Potholes_Processed, IRepository<LCMS_Potholes_Processed>>, IPotholesService 
    {
        private readonly AppDbContextProjectData _context;
        IDbContextFactory<AppDbContextProjectData> _dbContextFactory;

        public PotholesService(IRepository<LCMS_Potholes_Processed> repository, IDbContextFactory<AppDbContextProjectData> dbContextFactor) : base(repository)
        {
		...
		public async Task<IEnumerable<LCMS_Potholes_Processed>> QueryAsync(string predicate)
        {
            try
            {
                var sqlQuery = predicate;
                var lstTables = await _context.LCMS_Potholes_Processed.FromSqlRaw(sqlQuery).ToListAsync();
            ...
            catch (Exception ex)
            {
                ...
                return new List<LCMS_Potholes_Processed>();
            }
        }
        ...
        public async Task<IEnumerable<int>> QueryIdsAsync(string predicate)
        {
            ...
            {
                var sqlQuery = predicate;
                var ids = await _context.LCMS_Potholes_Processed.FromSqlRaw(sqlQuery)
                                                          .Select(item => item.Id)
                                                          .ToListAsync();
        ...
        public async Task<CountReply> GetCountAsync(string sqlQuery)
        {
            try
            {
                var count = await _context.LCMS_Potholes_Processed.FromSqlRaw(sqlQuery).CountAsync();
        ...
        public async Task<LCMS_Potholes_Processed> UpdateGenericData(string fieldsToUpdateSerialized)
        {
            var entity = new LCMS_Potholes_Processed();
