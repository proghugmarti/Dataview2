...
{
    public class MacroTextureService: BaseService<LCMS_Texture_Processed, IRepository<LCMS_Texture_Processed>>, IMacroTextureService
    {
        private readonly AppDbContextProjectData _context;
        IDbContextFactory<AppDbContextProjectData> _dbContextFactory;

        public MacroTextureService(IRepository<LCMS_Texture_Processed> repository, IDbContextFactory<AppDbContextProjectData> dbContextFactor) : base(repository)
        {
            _dbContextFactory = dbContextFactor;
            _context = _dbContextFactory.CreateDbContext();
        }

        public async Task<IEnumerable<LCMS_Texture_Processed>> QueryAsync(string predicate)
        {
            try
            {
                var sqlQuery = predicate;
                var lstTables = await _context.LCMS_Texture_Processed.FromSqlRaw(sqlQuery).ToListAsync();
            ...
            catch (Exception ex)
            {
                ...
                return new List<LCMS_Texture_Processed>();
            }
        }
        ...
        public async Task<CountReply> GetCountAsync(string sqlQuery)
        {
            try
            {
                var count = await _context.LCMS_Texture_Processed.FromSqlRaw(sqlQuery).CountAsync();
        ...
        public async Task<LCMS_Texture_Processed> UpdateGenericData(string fieldsToUpdateSerialized)
        {
            var entity = new LCMS_Texture_Processed();
